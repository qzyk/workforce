...
- rezolva inca `Pontaj`, `RaportActivitate`, `DocumentProiect`, `AngajatProiect` sau ierarhia BIM indirecta;
...
- `Pontaj` prin `Proiect` si `Angajat`
- `RaportActivitate` prin `Proiect` si `Angajat`
...
## Urmatoarele integrari recomandate

1. Proiect detail/edit/export: extindere catre datele nested din pagina, nu doar project lookup.
2. `RaportActivitate`: helper indirect prin `Proiect` si `Angajat`.
3. `Pontaj`: helper indirect prin `Proiect` si `Angajat`.
4. `Contract`: acces direct prin `tenant_id` si verificare cu `Proiect`.
5. `DocumentProiect` / `Document`: autorizare download si ownership indirect.
6. `ModelBIM`: acces prin `tenant_id` si `Santier`.
