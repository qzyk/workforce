...
@login_required
def lista():
    # request.args raman in ruta; asamblarea contextului tenant-safe (query,
    # filtre, sortare, paginare, statistici, manageri) sta in project_service.
    context = get_project_list_context(
    ...
    return render_template('proiecte/lista.html', **context)
...
def _get_total_ore(proiect_id):
    """Wrapper subtire peste project_service.get_project_total_hours.

    Pastrat pentru apelantii existenti din ruta (detalii).
    """
    return get_project_total_hours(proiect_id)


def _calculeaza_cost_manopera(proiect_id):
    """Wrapper subtire peste project_service.calculate_project_labor_cost."""
    return calculate_project_labor_cost(proiect_id)


def _get_ore_saptamanale(proiect_id, weeks=12):
    """Wrapper subtire peste project_service.get_project_weekly_hours."""
    return get_project_weekly_hours(proiect_id, weeks=weeks)


def _get_cost_lunar(proiect_id, months=6):
    """Wrapper subtire peste project_service.get_project_monthly_costs."""
    return get_project_monthly_costs(proiect_id, months=months)
