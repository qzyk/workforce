...
def _get_angajat_for_user(user):
    ...
    Wrapper subtire peste activity_service.get_current_employee_for_user, pastrat
    pentru compatibilitatea apelantilor existenti din ruta.
    """
    return get_current_employee_for_user(current_user=user)
...
def panou():
    ...
    # construit in activity_service. Ruta ramane responsabila doar de HTTP.
    filters = {
        'angajat_id': request.args.get('angajat_id', '', type=int) or None,
        'proiect_id': request.args.get('proiect_id', '', type=int) or None,
        'tip_instalatie_id': request.args.get('tip_instalatie_id', '', type=int) or None,
        'status': request.args.get('status', ''),
        'tip': request.args.get('tip', ''),  # tip_activitate: zilnica/saptamanala/lunara
        'status_executie': request.args.get('status_executie', ''),
        'data_start': request.args.get('data_start', ''),
        'data_end': request.args.get('data_end', ''),
        'santier_id': request.args.get('santier_id', type=int) or None,
        'cladire_id': request.args.get('cladire_id', type=int) or None,
        'element_bim_id': request.args.get('element_bim_id', type=int) or None,
        'tip_element': request.args.get('tip_element', '').strip(),
    }
    context = get_activity_panel_context(filters=filters, current_user=current_user)
    return render_template('activitati/panou.html', **context)
...
def adauga():
    ...
    context = get_activity_form_context(activity=None, current_user=current_user)
...
def editeaza(id):
    ...
    context = get_activity_form_context(activity=activitate, current_user=current_user)
    return render_template('activitati/formular.html',
        activitate=activitate,
        today=date.today(),
        angajat_curent=angajat_curent,
